...
export async function sendConfirmationEmail(data: BookingEmailData) {
  const ridersList = data.riders
    .map(r => `${r.name} (${r.age} jr, ${r.weight} kg, ${r.type === 'adult' ? 'volwassene' : 'kind'})`)
    .join('\n    ')
...
<body>
  <div class="header">
    <h1>${data.productIcon} ${data.productName}</h1>
    <p>${dateFormatted}</p>
  </div>
  <div class="body">
    <div class="detail">
      <div class="label">Starttijd</div>
      <div class="value">${data.startTime} uur (${data.duration} minuten)</div>
    </div>
    <div class="detail">
      <div class="label">Aanwezig om</div>
      <div class="value" style="color: #CC4444; font-weight: bold;">${data.arriveTime} uur</div>
    </div>
    <div class="detail">
      <div class="label">Ruiters</div>
      ${data.riders.map(r => `
        <div class="rider">
          <strong>${r.name}</strong> · ${r.age} jr · ${r.weight} kg · ${r.type === 'adult' ? 'Volwassen paard' : 'Kinderpaard'}
        </div>
      `).join('')}
    </div>
    <div class="detail">
  ...
  return resend.emails.send({
    ...
    to: data.to,
    subject: `Bevestiging ${data.productName} — ${dateFormatted}`,
    html,
  })
}
